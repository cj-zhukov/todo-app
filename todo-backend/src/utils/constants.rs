...
pub static DB_NAME_SECRET: LazyLock<String> = LazyLock::new(|| {
    dotenv().ok();
    let secret = std_env::var(env::DB_NAME_ENV_VAR).expect("DB_NAME_SECRET must be set.");
    if secret.is_empty() {
...
pub static DB_USER_SECRET: LazyLock<String> = LazyLock::new(|| {
    dotenv().ok();
    let secret = std_env::var(env::DB_USER_ENV_VAR).expect("DB_USER_NAME_SECRET must be set.");
    if secret.is_empty() {
...
pub static PASSWORD_SECRET: LazyLock<String> = LazyLock::new(|| {
    dotenv().ok();
    let secret = std_env::var(env::PASSWORD_ENV_VAR).expect("PASSWORD_SECRET must be set.");
    if secret.is_empty() {
